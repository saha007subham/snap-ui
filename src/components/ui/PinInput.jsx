...
export const PinInput = forwardRef(
  (
    ...
    // Map value to array representation
    ...
    // Keep internal values synchronised if controlled changes
    useEffect(() => {
      if (isControlled) {
        setInternalValue(
          (customValue || "").split("").concat(Array(length).fill("")).slice(0, length)
    ...
    // Expose DOM elements and state management via ref
    ...
    // Input filtering based on validation type
    ...
    const handleInputChange = (e, index) => {
      ...
      if (!char) return; // Handled in keydown backspace

      // Take only the last entered char in case of weird autocomplete
    ...
    const handleKeyDown = (e, index) => {
      if (e.key === "Backspace") {
        ...
        if (valuesArray[index]) {
          // Clear current character but keep focus
          newValues[index] = "";
          triggerChanges(newValues);
        } else if (index > 0) {
          // Move backward and clear that character
          newValues[index - 1] = "";
          triggerChanges(newValues);
          inputRefs.current[index - 1]?.focus();
        }
    ...
    const handlePaste = (e) => {
      ...
      // Clean pasted data matching character restriction rules
      ...
      // Focus last pasted or complete slot
      const focusIndex = Math.min(cleaned.length, length - 1);
      inputRefs.current[focusIndex]?.focus();
    };

    // Calculate inline gap styles in case user inputs arbitrary numbers
            ...
            return (
              <div key={index} className="relative">
                <input
                  ...
                  onPaste={index === 0 ? handlePaste : undefined} // paste enabled globally on first slot or generic bubble
                ...
                {/* Beautiful Mask custom element for smooth transitions */}
                {mask && hasVal && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
